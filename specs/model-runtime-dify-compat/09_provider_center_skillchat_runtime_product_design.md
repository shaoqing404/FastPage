...
不做 `Custom headers` auth mode。内网无密码资源用 `No auth` 显式表达；需要 API key 的资源用 `API key` 表达。Dify 或更复杂 header-based auth 以后单独设计，不进入本轮 Provider Center 基础产品形态。
