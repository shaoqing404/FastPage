...
- the follow-up frontend continuity fixes required by real usage are also landed:
  - KB direct upload now triggers parse
  - skill-chat assistant timestamps no longer use `run.created_at`
  - shared modal centering no longer drifts under motion transforms
