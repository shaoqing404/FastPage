...
- [README.md](spec/fastapi_service/phase3_service_productization/README.md)
- [tenant_and_workspace_model.md](spec/fastapi_service/phase3_service_productization/tenant_and_workspace_model.md)
- [phase3_frontend_closeout_report.md](spec/fastapi_service/phase3_service_productization/phase3_frontend_closeout_report.md)
- [auth.py](app/core/auth.py)
- [bootstrap.py](app/core/bootstrap.py)
...
- `Workspace` exists as a first-class table with `tenant_id`, `slug`, `is_default`, and `default_provider_id`: [workspace.py](app/models/workspace.py#L9)
- `TenantMembership` exists as a first-class table with `tenant_id`, `user_id`, `role`, and `status`: [tenant_membership.py](app/models/tenant_membership.py#L9)
- `Principal` now carries `tenant_id`, `workspace_id`, and `membership_role`: [principal.py](app/core/principal.py#L6)
- login/auth resolves membership plus active workspace and includes them in JWT claims: [auth.py](app/core/auth.py#L122), [auth.py](app/core/auth.py#L165)
- bootstrap guarantees one default workspace plus one owner membership for the bootstrap tenant: [bootstrap.py](app/core/bootstrap.py#L44), [bootstrap.py](app/core/bootstrap.py#L70), [bootstrap.py](app/core/bootstrap.py#L118)
...
- login returns exactly one resolved workspace context plus a flat membership list, but no switch API: [auth.py](app/api/routers/auth.py#L29)
- `resolve_auth_context(...)` always picks one active membership and then one workspace, defaulting to the tenant default workspace when no explicit workspace is supplied: [auth.py](app/core/auth.py#L79), [auth.py](app/core/auth.py#L94), [auth.py](app/core/auth.py#L122)
- frontend stores the returned `workspace` and `memberships` in local storage, but active workspace resolution is just `user.workspace_id`; there is no UI or API contract for switching: [LoginPage.tsx](frontend/src/pages/LoginPage.tsx#L20), [client.ts](frontend/src/lib/api/client.ts#L141)
...
- membership lookup is the access-control source used by auth: [auth.py](app/core/auth.py#L79), [auth.py](app/core/auth.py#L122)
- login returns memberships to the client: [auth.py](app/api/routers/auth.py#L36)
...
The router package only exposes auth, documents, jobs, skills, chat, providers, metrics, knowledge bases, and compliance routes. There is no workspace lifecycle router, no membership router, and no user admin router under [app/api/routers](app/api/routers).
...
- `User`: identity row, still carrying compatibility `tenant_id`: [user.py](app/models/user.py#L9)
- `TenantMembership`: tenant access + role: [tenant_membership.py](app/models/tenant_membership.py#L9)
- `membership_role` in principal and token: [principal.py](app/core/principal.py#L6), [auth.py](app/core/auth.py#L165)
...
| --- | --- | --- | --- |
| `document` | `workspace_id` exists and is nullable: [document.py](app/models/document.py#L9) | read/write uses principal workspace filter, with default-workspace fallback to legacy `NULL` rows: [document_service.py](app/services/document_service.py#L23), [documents.py](app/api/routers/documents.py#L68) | workspace-aware, but transitional |
| `provider` | `workspace_id` exists and is nullable: [model_provider.py](app/models/model_provider.py#L9) | runtime binding checks workspace accessibility, but CRUD/list routes remain tenant-scoped and schema does not expose workspace ownership: [provider_service.py](app/services/provider_service.py#L67), [provider_service.py](app/services/provider_service.py#L324), [providers router](app/api/routers/providers.py#L18), [providers schema](app/schemas/providers.py#L8) | tenant-aware with workspace hook, not a true workspace-managed resource |
| `knowledge base` | `workspace_id` is required: [knowledge_base.py](app/models/knowledge_base.py#L9) | all CRUD routes are `/workspaces/{workspace_id}/...` and require current principal workspace to match: [knowledge_bases.py](app/api/routers/knowledge_bases.py#L29), [knowledge_base_service.py](app/services/knowledge_base_service.py#L15) | strongly workspace-aware inside the current active workspace |
| `skill` | `workspace_id` exists and is nullable: [chat_skill.py](app/models/chat_skill.py#L9) | create/list/get/update validate same-workspace docs/provider/KB and list by `principal.workspace_id`: [skill_service.py](app/services/skill_service.py#L58), [skill_service.py](app/services/skill_service.py#L129), [skills.py](app/api/routers/skills.py#L22) | workspace-aware |
| `session` | `workspace_id` exists and is nullable: [chat_session.py](app/models/chat_session.py#L9) | create uses current workspace; list/get filter by workspace with default-workspace fallback to `NULL`: [session_service.py](app/services/session_service.py#L22), [session_service.py](app/services/session_service.py#L42), [chat.py](app/api/routers/chat.py#L256) | workspace-aware, but transitional |
| `run` | `workspace_id` exists and is nullable: [chat_run.py](app/models/chat_run.py#L9) | run creation derives workspace from session/document/skill; reads filter by workspace with default-workspace fallback: [chat_service.py](app/services/chat_service.py#L292), [chat_service.py](app/services/chat_service.py#L354), [chat.py](app/api/routers/chat.py#L203) | workspace-aware, but transitional |
| `api key` | `workspace_id` exists and is nullable: [api_key.py](app/models/api_key.py#L9) | issuance/list/revoke are all tied to the current principal workspace; auth resolves API key workspace context: [auth.py](app/api/routers/auth.py#L84), [auth.py](app/api/routers/auth.py#L117), [auth.py](app/core/auth.py#L223) | workspace-aware |
| `parse job` | `workspace_id` exists and is nullable: [parse_job.py](app/models/parse_job.py#L9) | creation records workspace, but list/get routes ignore active workspace and only filter by `current_user.tenant_id`: [documents.py](app/api/routers/documents.py#L110), [jobs.py](app/api/routers/jobs.py#L13) | still tenant-aware in exposed API |
...
- bootstrap creates and maintains only one default tenant/user/workspace shell: [bootstrap.py](app/core/bootstrap.py#L118)
- `jobs` uses `get_current_user()` and scopes by `current_user.tenant_id`, not principal workspace: [deps.py](app/api/deps.py#L11), [jobs.py](app/api/routers/jobs.py#L13)
- `metrics/overview` also uses `get_current_user()` and aggregates tenant-wide counts: [metrics.py](app/api/routers/metrics.py#L13)
- provider CRUD/list stays tenant-scoped even though the model carries `workspace_id`: [providers.py](app/api/routers/providers.py#L18), [provider_service.py](app/services/provider_service.py#L56)
...
- documents: [document_service.py](app/services/document_service.py#L39)
- skills: [skill_service.py](app/services/skill_service.py#L58)
- runs: [chat_service.py](app/services/chat_service.py#L544)
- providers: [provider_service.py](app/services/provider_service.py#L56)
- storage layout remains tenant-rooted: tenant data is stored under `tenants/{tenant_id}` paths in storage service: [storage_service.py](app/services/storage_service.py#L47)
...
- `User.tenant_id` remains mandatory compatibility state: [user.py](app/models/user.py#L12)
- auth still falls back to `user.tenant_id` when resolving context: [auth.py](app/core/auth.py#L129)
- `get_current_user()` throws away principal tenant/workspace context and returns only the raw `User`, which means downstream routes can silently scope by `user.tenant_id` instead of active membership context: [deps.py](app/api/deps.py#L11)
- `jobs` and `metrics` already do this: [jobs.py](app/api/routers/jobs.py#L13), [metrics.py](app/api/routers/metrics.py#L13)
- there is still no tenant lifecycle or tenant switching API even though memberships exist: [tenant_and_workspace_model.md](spec/fastapi_service/phase3_service_productization/tenant_and_workspace_model.md)
...
- knowledge bases and compliance resources are explicitly nested under `/workspaces/{workspace_id}` and reject mismatched workspace context: [knowledge_bases.py](app/api/routers/knowledge_bases.py#L29), [compliance_checks.py](app/api/routers/compliance_checks.py#L20), [compliance_runs.py](app/api/routers/compliance_runs.py#L18), [knowledge_base_service.py](app/services/knowledge_base_service.py#L15)
- documents, sessions, and runs do apply workspace filtering in services: [document_service.py](app/services/document_service.py#L33), [session_service.py](app/services/session_service.py#L22), [chat_service.py](app/services/chat_service.py#L292)
- API keys are issued and listed per active workspace: [auth.py](app/api/routers/auth.py#L84), [auth.py](app/api/routers/auth.py#L117)
...
- default workspace can see legacy `NULL workspace_id` rows for documents/sessions/messages/runs, so behavior is intentionally compatibility-driven rather than strictly partitioned: [document_service.py](app/services/document_service.py#L33), [session_service.py](app/services/session_service.py#L22), [chat_service.py](app/services/chat_service.py#L292)
- there is no workspace list API, no switch API, and no workspace CRUD API, so users cannot actually operate multiple workspaces even though the runtime can carry one: [auth.py](app/api/routers/auth.py#L29), [client.ts](frontend/src/lib/api/client.ts#L163)
- provider ownership is ambiguous: runtime checks workspace accessibility, but CRUD/list remains tenant-wide and schema does not expose `workspace_id`: [provider_service.py](app/services/provider_service.py#L67), [providers router](app/api/routers/providers.py#L31), [providers schema](app/schemas/providers.py#L28)
- `jobs` and `metrics` are not workspace-aware surfaces at all: [jobs.py](app/api/routers/jobs.py#L13), [metrics.py](app/api/routers/metrics.py#L13)
- chat session/run API schemas do not expose `workspace_id`, which makes cross-workspace admin or diagnostics difficult even if the DB field exists: [chat schema](app/schemas/chat.py#L31)
...
| --- | --- | --- | --- |
| workspace list | No | no workspace router; frontend only reads one stored `workspace`: [client.ts](frontend/src/lib/api/client.ts#L152) | can only operate current workspace context |
| workspace switch | No | login returns one resolved workspace, but no `/auth/context/switch` or equivalent route: [auth.py](app/api/routers/auth.py#L29) | memberships are informational only |
| workspace create/update/delete | No | no workspace CRUD router in [app/api/routers](app/api/routers) | workspace is not an operable product object |
| workspace member list | No | no workspace member router/model | cannot present workspace access administration |
| add/invite/remove member | No | `TenantMembership` exists, but no member mutation routes: [tenant_membership.py](app/models/tenant_membership.py#L9) | no team onboarding/offboarding flow |
| role change | No | role is stored in membership, but there is no API to mutate it: [tenant_membership.py](app/models/tenant_membership.py#L18) | admin roles cannot be managed |
...
| enable/disable user | No surface | `User.is_active` exists, but there is no admin API: [user.py](app/models/user.py#L16) | deactivation is only a data-model possibility, not a product feature |

### 2. Product effect on the frontend

These gaps directly explain the current frontend limitations:

- the frontend can display only the currently stored workspace context, not manage or switch it: [LoginPage.tsx](frontend/src/pages/LoginPage.tsx#L20), [client.ts](frontend/src/lib/api/client.ts#L163)
- `memberships` are stored client-side but not consumable through any follow-up backend workflow: [LoginPage.tsx](frontend/src/pages/LoginPage.tsx#L22)
- the “Workspace console” IA from Phase 3 frontend closeout is therefore a single active-workspace shell, not a real multi-workspace admin product yet: [phase3_frontend_closeout_report.md](spec/fastapi_service/phase3_service_productization/phase3_frontend_closeout_report.md)
...
- [workspace.py](app/models/workspace.py#L9)
- [tenant_membership.py](app/models/tenant_membership.py#L9)
- [user.py](app/models/user.py#L9)
...
- [principal.py](app/core/principal.py#L6)
- [auth.py](app/core/auth.py#L122)
- [auth.py](app/api/routers/auth.py#L29)
...
- the current token already carries `workspace_id`: [auth.py](app/core/auth.py#L165)
- the frontend already stores workspace context: [LoginPage.tsx](frontend/src/pages/LoginPage.tsx#L22)
...
- Knowledge Base: workspace-owned and explicit: [knowledge_base.py](app/models/knowledge_base.py#L9)
- Skill: workspace-owned in service behavior: [skill_service.py](app/services/skill_service.py#L129)
- API key: workspace-owned in auth behavior: [auth.py](app/api/routers/auth.py#L84)
