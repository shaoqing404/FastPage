...
DEFAULT_SKILL_FOLLOWUP_QUESTION = "请再补一句，说明文档最核心的结论。"
...
def run_validation(args: argparse.Namespace) -> dict[str, object]:
    ...
    try:
        ...
        checks["direct_query"] = {
            ...
            "session_id": direct_run.get("session_id"),
        }
        ...
        created["session_id"] = skill_run.get("session_id")
        ...
        _assert(bool(created["session_id"]), "skill run did not create a reusable session")
        checks["session_create"] = {
            "session_id": created["session_id"],
            "source": "auto_create_session on first skill run",
        }
        checks["skill_run"] = {
        ...
        _, skill_run_followup = _request(
            "POST",
            _path(args.base_url, f"/api/v1/chat/skills/{created['skill_id']}/run"),
            token=user_token,
            json_payload={
                "question": DEFAULT_SKILL_FOLLOWUP_QUESTION,
                "document_id": created["document_id"],
                "provider_id": created["provider_id"],
                "session_id": created["session_id"],
                "generation_config": {"temperature": 0},
        ...
        _assert(isinstance(skill_run_followup, dict), "skill follow-up run did not return JSON")
        created["skill_run_followup_id"] = skill_run_followup["id"]
        _assert(skill_run_followup["status"] == "completed", f"skill follow-up run not completed: {skill_run_followup['status']}")
        _assert(skill_run_followup.get("session_id") == created["session_id"], "skill follow-up did not reuse the original session")
        checks["skill_run_followup"] = {
            "run_id": created["skill_run_followup_id"],
            "status": skill_run_followup["status"],
            "citation_count": len(skill_run_followup.get("citations", [])),
            "session_id": created["session_id"],
        }

        _, messages_payload = _request(
            "GET",
            _path(args.base_url, f"/api/v1/chat/skills/{created['skill_id']}/sessions/{created['session_id']}/messages"),
            token=user_token,
        )
        ...
        checks["session_messages"] = {
            "count": len(messages_payload),
            "user_messages": sum(1 for item in messages_payload if item.get("role") == "user"),
            "assistant_messages": sum(1 for item in messages_payload if item.get("role") == "assistant"),
        }
